...
class Render(torch.autograd.Function):
    ...
    def forward(ctx, vertices, faces, textures=None):
        ...
        imgs = _renderer_.forward_img(vs, fs, ts)
        return convert_as(torch.Tensor(imgs), vertices)

    @staticmethod
    def backward(ctx, grad_out):
        g_o = grad_out.cpu().numpy()
        grad_verts, grad_tex = _renderer_.backward_img(g_o)
...
class NeuralRenderer(torch.nn.Module):
    ...
    """
    def __init__(self, img_size=800, device=0):
        super(NeuralRenderer, self).__init__()
        global _renderer_
        _renderer_ = NMR()
        
        # rendering
        _renderer_.renderer.image_size = img_size
        
        # camera
        _renderer_.renderer.camera_mode = 'look'
        _renderer_.renderer.viewing_angle = 45
        eye, camera_direction, camera_up = get_params(((-25, 16, 20), (-45, 180, 0)), ((-45, 3, 0.8), (0, 0, 0)))  # test example
        _renderer_.renderer.eye = eye
        _renderer_.renderer.camera_direction = camera_direction
        _renderer_.renderer.camera_up = camera_up
        
        # light
        _renderer_.renderer.light_intensity_ambient = 0.5
        _renderer_.renderer.light_intensity_directional = 0.5
        _renderer_.renderer.light_color_ambient = [1, 1, 1]  # white
        _renderer_.renderer.light_color_directional = [1, 1, 1]  # white
        _renderer_.renderer.light_direction = [0, 0, 1]  # up-to-down

        _renderer_.to_gpu(device=device)
    ...
    def ambient_light_only(self):
        ...
        _renderer_.renderer.light_intensity_ambient = 1
        _renderer_.renderer.light_intensity_directional = 0

    def set_bgcolor(self, color):
        global _renderer_
        _renderer_.renderer.background_color = color

    def project_points(self, verts, cams):
        proj = self.proj_fn(verts, cams)
        return proj[:, :, :2]

    def set_param(self, eye, camera_direction, camera_up):
        global _renderer_
        _renderer_.renderer.eye = eye
        _renderer_.renderer.camera_direction = camera_direction
        _renderer_.renderer.camera_up = camera_up
    ...
    def forward(self, vertices, faces, textures=None):
        return self.RenderFunc.apply(vertices, faces, textures)


def example():
    obj_file = 'test.obj'
    data_path = '/root/cgj/DualAttentionAttack/src/data/phy_attack/train/data132.npz'
