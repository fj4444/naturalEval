...
class LPIPSDataset(Dataset):
    def __init__(self, data_dir, label_file, split_str, img_size, device=0, gaze_dir=''):
        ...
        self.file_list = []
        self.companion_file_list = []
        for index_item in self.files.keys():
            self.file_list.append(index_item)
        for index_item in self.files.keys():
            total_list = index_item.split("/")
            total_list[4] = "new-final-baseline0"
            prefix = "/".join(total_list[:-1])
            suffix = total_list[-1]
            suffix_list = suffix.split("-")
            right_file = None
            for _, _, files in os.walk(prefix, topdown=False):
                for file in files:
                    file_list = file.split("-")
                    if file_list[5] == suffix_list[5] and file_list[1] == suffix_list[1]:
                        right_file = file
                        break
            self.companion_file_list.append(right_file)
    ...
    def __getitem__(self, index):
        ...
        companion_img = self.open_and_process_image(self.companion_file_list[index])
        gaze_score = self.cal_score(file_list)
        return index, gaze_img, gaze_score, original_img, companion_img
