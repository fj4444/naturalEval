...
from grad_cam_densenet import CAM_DENSENET
...
parser.add_argument("--model", type=int, default=0) # 0 resnet 1 densenet
...
def loss_gaze(gold_map, pred_map):
    return torch.mean((gold_map - pred_map) ** 2)
...
def run_cam(data_dir, epoch, train=True, batch_size=BATCH_SIZE):
    print(data_dir)
    dataset = MyDataset(data_dir, 224, texture_size, faces, vertices, distence=None, mask_dir=mask_dir, ret_mask=True, device=0, gaze_dir=gaze_dir)
    loader = DataLoader(
        dataset=dataset,     
    ...
    if MODEL == 0:
        Cam_heatmap = CAM(device=0)
    elif MODEL == 1:
        Cam_heatmap = CAM_DENSENET()
    ...
    for _ in range(epoch):
        ...
        for i, (index, total_img, texture_img, masks, heat_map) in enumerate(tqdm_loader): # 加上 heatmap
            index = int(index[0])
            ...
            _, _, gaze_map = Cam_heatmap(total_img, index, log_dir)
            mask, pred, _ = Cam(total_img, index, log_dir)
            # ipdb.set_trace()
            ...
            loss_gaz = loss_gaze(heat_map[0], gaze_map)
            loss = loss_midu(mask) + lamb * loss_content_diff(texture_param) + loss_smooth(texture_img, masks) + loss_gaz
            
            
            with open(os.path.join(log_dir, 'loss.txt'), 'a') as f:
            
                tqdm_loader.set_description('Loss %.8f, Prob %.8f, Loss Gaze %.8f' % (loss.data.cpu().numpy(), pred, loss_gaz.data.cpu().numpy()))
...
if __name__ == '__main__':
    ...
    logs = {
        'model': MODEL,
        'pattern': args.content.split('/')[-1][:-4],
        'epoch': EPOCH
    }
