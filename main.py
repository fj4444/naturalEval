...
def train(model, loader, optimizer, device):
    ...
    for read_cnt, (data, label) in enumerate(loader):
        for idx in range(0, data.shape[0]):
            ...
            #   continue
            optimizer.zero_grad()
            _, result, loss, loss1 = model(data[idx].cuda())
            # ipdb.set_trace()
            y = label[idx].long().cuda()
            # ipdb.set_trace()
            loss2 = F.nll_loss(result, y)
            
            loss1 = torch.Tensor(np.array(loss1))
...
def main():
    ...
    train_dataset, eval_dataset = get(batch_size=1, data_root='.', train=True, val=True)
